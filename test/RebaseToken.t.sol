...
contract RebaseTokenTest is Test {
    ...
    function testGetRebaseTokenAddresss() public view {
        assertEq(vault.getRebaseTokenAddress(), address(rebaseToken));
    }

    function testInteresRateCanOnlyDecrease(uint256 newInterestRate) public {
        uint256 initialInterestRate = rebaseToken.getInterestRate();
        newInterestRate = bound(newInterestRate, initialInterestRate, type(uint96).max);
        vm.prank(owner);
        vm.expectPartialRevert(bytes4(RebaseToken.RebaseToken_InteresRateCanOnlyDecrease.selector));
        rebaseToken.setInteresRate(newInterestRate);
